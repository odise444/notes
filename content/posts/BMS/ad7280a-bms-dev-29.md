...
이거 보면서 "아 이 정도면 금방 하겠네" 싶었는데, 평가보드는 6셀짜리 싱글 디바이스였다. 나는 24셀이라 4개를 데이지체인으로 연결해야 했다.

![데이지체인 구성](/imgs/ad7280a-bms-dev-01-2.png)

첫 번째 삽질은 Figure 29가 **단일 디바이스**용이라는 거였다. 그림 밑에 작은 글씨로 "For multiple devices, refer to daisy-chain configuration"이라고 써있더라. 이걸 왜 이렇게 작게 써놨는지.
...
6월 초에 드디어 실제 셀에 연결해서 테스트했다.

![6셀 테스트](/imgs/6cells.jpg)

일단 6셀로 먼저 검증하고, 문제없으면 12셀로 확장했다.

![12셀 연결](/imgs/12cells.jpg)

셀 탭 연결할 때 순서 꼬이면 난리난다. 이거 정리하는 것도 일이었다.

![셀 탭 연결](/imgs/cells%20connection.jpg)

---

최종적으로 셀 전압 오차 ±5mV 이내, 밸런싱 정상, 데이지체인 4개 다 인식, 모터 돌려도 노이즈 ±10mV.
